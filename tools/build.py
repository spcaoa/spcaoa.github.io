...
VER=_assetver()
def _pagever():
    # changes whenever any input to the published pages changes: generators, data, assets
    import hashlib,glob as _g
    h=hashlib.sha256()
    files=sorted(_g.glob(os.path.join(ROOT,"tools","*.py"))+_g.glob(os.path.join(DATA,"*"))+_g.glob(os.path.join(MC,"*.csv"))+_g.glob(os.path.join(OUT,"assets","*")))
    for f in files:
        if os.path.isfile(f): h.update(open(f,"rb").read())
    return h.hexdigest()[:10]
PAGEVER=_pagever()
with open(os.path.join(OUT,"version.json"),"w") as _vf: _vf.write('{"v":"'+PAGEVER+'"}')
...
lhealth=json.load(open(os.path.join(DATA,"lifts_health.json"))); lifts=rd(os.path.join(DATA,"lifts.csv")); comps=rd(os.path.join(DATA,"complaints.csv")); lsvc=rd(os.path.join(DATA,"lifts_service.csv"))
...
def page(fn,title,body,scripts=""):
    ...
    html=f'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow"><meta http-equiv="Cache-Control" content="no-cache, no-store, must-revalidate"><meta http-equiv="Pragma" content="no-cache"><meta http-equiv="Expires" content="0">
<script>(function(){{var V="{PAGEVER}";try{{var q=new URLSearchParams(location.search);fetch("version.json?t="+Date.now(),{{cache:"no-store"}}).then(function(r){{return r.json()}}).then(function(j){{if(j.v&&j.v!==V&&q.get("v")!==j.v){{q.set("v",j.v);location.replace(location.pathname+"?"+q.toString()+location.hash)}}}}).catch(function(){{}})}}catch(e){{}}}})();</script>
...
<li>The reserves are about {L(reserves)}. Their interest already goes on running costs — though about ₹34 L a year of it sits inside the ICICI deposits until they mature, so it is earned but not in the bank. Last year ₹50 L of the reserves themselves was used as well, and this year another ₹38 L so far.</li>
...
<section class="split"><div class="body"><h2>Why the bank still runs dry</h2>
<p>The ₹84 L above counts every rupee of interest the reserves earn. But not all of that interest actually reaches the bank. HDFC pays its interest into the operating account every month, about ₹25.6 L a year, and Sobha pays about ₹6 L. The ICICI deposits — ₹5.27 Cr, the largest block — add their interest back into the deposit instead. That is about ₹34 L a year which the association has earned, and pays tax on, but does not receive until the deposit matures.</p>
<p>So the cash the association actually has to find is not ₹84 L but about <strong>₹118 L a year</strong>, and the difference is made up by breaking deposits. The 39% tax on the ICICI interest is paid in cash too, out of those same broken deposits, years before the interest itself arrives.</p>
<p>This part can be fixed without asking anyone for money. ICICI offers the same rate with the interest paid out monthly instead of added to the deposit. Switching would bring about ₹2.86 L a month into the account. It does not earn a rupee more, and it does not make the reserves last any longer — it stops the association breaking good deposits to fund interest it has already earned. Deposits worth about ₹41 L mature within the year and can simply be re-booked; the rest carry a 1% pre-closure charge, roughly ₹4–5 L once. The Managing Committee is taking this up with the bank.</p>
<p class="small">The two figures answer different questions. ₹84 L is what the reserves lose in a year. ₹118 L is what has to be found in cash to pay the bills. Once the ICICI deposits pay out monthly, the two become the same number.</p></div>
<aside class="side"><h3>Interest, and where it goes</h3><dl>
<dt>HDFC — paid in every month</dt><dd>₹25.6 L</dd>
<dt>Sobha — paid in every month</dt><dd>₹6.0 L</dd>
<dt>ICICI — added to the deposit</dt><dd class="up">₹34.3 L</dd>
<dt>Tax paid on all of it</dt><dd>₹25 L</dd>
<dt>Gap on paper</dt><dd>₹84 L</dd>
<dt>Cash to find</dt><dd class="key">₹118 L</dd>
</dl><p class="note">Rates and balances as at 14 September 2026. The tax is paid on all the interest, including the part that has not been received.</p></aside></section>
...
<p><strong>The Finance Sub-Committee's 10.1%.</strong> The Finance Sub-Committee, the owners who reviewed last year's accounts, proposed 10.1%. Two things go wrong with it. First, their 10.1% is applied to the bill with the GST inside it. For the 148 flats that cross ₹7,500 the tax comes out of that same amount, so the association keeps less than it does today: an A-type flat would pay ₹8,183, of which ₹1,248 is GST, leaving ₹6,934 against ₹7,432 now. Across the estate the association ends up about ₹2 L a year better off, not ₹23 L. Second, their ₹23 L gap was small because it left out one-off repairs, counted ₹18 L of savings before any had been made, and assumed about half the tax the association actually pays. Even 10.1% charged on the base, with GST added on top, would raise ₹23 L of the ₹64 L needed. Neither figure counts the GST the association can claim back on its own bills, which is much the same under either proposal.</p>
...
lhrows="".join(f'<tr><td>{r["wing"]} · {r["type"].lower()} <span class="small">({r["lift"]})</span></td><td class="num{" neg" if float(r["availability"])<99.3 else ""}">{r["availability"]}%</td><td>{r["last_callback"]}</td><td>{("<strong>"+r["status_plain"]+"</strong>") if not r["status_plain"].startswith("Running") else r["status_plain"]}</td><td>{r["report"]}</td></tr>' for r in lhealth)
...
<section><h2>Lift uptime</h2><p>Lifts were the lowest-rated service (3.30 of 5) and the top priority for 54% of households. The lift contract costs ₹16 L a year. Batteries cost ₹6.75 L last year.</p>
<p>Schindler monitors every lift remotely and has sent a health check for each. Its figures are below, as received.</p>
<div class="tablewrap"><table class="svc"><thead><tr><th>Lift</th><th class="num">Availability</th><th>Last breakdown call</th><th>At the time of the report</th><th>Report date</th></tr></thead><tbody>{lhrows}</tbody></table></div>
<p class="small">Figures are Schindler’s own, from its remote monitoring. The reports do not say what period the availability covers, and they do not record how long each breakdown lasted. “Last breakdown call” is the most recent time a fault was called in. Both Wing 2 lifts were out of normal service when their reports were generated, which matches where most of the complaints come from. <a href="docs/Schindler-lift-health-checks-Aug-Sep-2026.zip">Download all eight reports (zip, 2 MB)</a>.</p>
<h3>Breakdown log</h3>{lifts_html()}</section>
...
("Isn't ₹9.7 crore in reserves enough to carry on?","It would carry on for a few years and then be gone, and it is the fund for the big jobs ahead — lift replacement and overhaul, painting, waterproofing, the sewage plant and the rest. Its interest, about "+L(int_yr)+" a year before tax, is already committed to running costs, though about ₹34 L of that is added back into the ICICI deposits rather than paid to us until they mature. Spending the principal too is what last year's Managing Committee did, and what the General Body objected to."),
