...
<p class="lead">Pick an option and see what your bill becomes and whether the association balances. Move the slider or change an assumption and everything updates. If nothing changes, <b id="gapInline"></b> comes out of the reserves between September and March, and next year is still short.</p></section>
<div class="simgrid">
<aside class="simpanel" id="simpanel" aria-label="Options and assumptions">
  ...
  <div class="ctls">
    ...
    <div class="ctl"><div class="ctl-h">GST</div>
      <div class="ctl-note">Once a flat's charge passes ₹7,500 a month, 18% applies to the whole amount.</div>
      <label><input type="checkbox" id="itc"> Association claims back GST on its own bills</label></div>
...
function render(){
  ...
  list.innerHTML=KEYS.map(k=>{ const x=R[k], sel=k===o.preset;
    ...
    return row+'<div class="odetail"><div class="big" data-f="bill">'+inr(x.a.total)+'<span> /month, A-type flat</span></div>'
      ...
      +'<div class="kv"><span>From reserves, Sep–Mar</span><b class="'+(x.fromRes>0?"neg":"pos")+'" data-f="res">'+L(x.fromRes)+'</b></div>'
