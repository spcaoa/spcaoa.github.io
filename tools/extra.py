...
def build_extra(page, CH, runway):
    ...
    sim = """<section class="simhead"><div class="eyebrow">Six options, one model</div><h1>Try the options</h1>
...
<aside class="simpanel" id="simpanel" aria-label="Options and assumptions">
  <div class="live" id="live" aria-live="polite">
    <div class="live-h"><span>Option</span><span>Next year</span></div>
    <div id="optList"></div>
  </div>
  <div class="ctls">
    <div class="ctl"><div class="ctl-h">MC option: base increase <b id="pctLabel">20</b>%</div>
      <input type="range" id="pct" min="0" max="45" step="1" value="20" aria-label="MC option base increase">
      <div class="ctl-note">Moving this selects the MC option.</div></div>
    ...
    <div class="ctl"><div class="ctl-h">When the new rate starts</div>
      ...
      <label><input type="checkbox" id="rep2" checked> Option 2 repeated next year</label></div>
    <div class="ctl"><div class="ctl-h">Costs</div>
      <div class="ctl-sub">Savings from tenders, from Jan 2027</div>
      ...
      <select id="tax"><option value="4">₹4 L a quarter (as paid in September)</option><option value="6.3" selected>₹6.3 L a quarter (39% on all interest)</option></select></div>
...
<p class="small">"This year" runs from the real end-of-August cash position through March 2027. "Next year" is April 2027 to March 2028 on the same basis as the shortfall page: this year's run-rate including ₹40 L of one-time items, 92% collection, all interest counted, tax at 39%.</p></section>
...
<section><h2>How the model works</h2><ol class="steps"><li>Spending: ₹26 L a month, plus ₹8 L for the lift contract in June and December, plus tax each quarter. Or ₹33 L flat, the Treasurer's figure. Savings reduce it from January 2027.</li><li>Income in a quiet month: ₹1.8 L late maintenance, ₹0.8 L rentals and fees, ₹2.1 L HDFC interest, ₹0.5 L Sobha interest. In a billing month: 92% of the quarter's bills.</li><li>GST collected goes to the government. If input credit is on, the association keeps its own vendor GST in proportion to taxable billing.</li><li>Option 2 nets ₹69.5 L after GST, collected in November. The Sub-Committee's 10.1% is applied to the bill with GST inside it, as in their report.</li><li>Next year counts collections at 92% of billing, adds ₹40 L of one-time items at this year's rate, adds ICICI interest (₹34 L at 6.5%) and deducts tax on all interest at 39%. GST input credit is off unless you switch it on; it is worth up to ₹35 L a year but the tax adviser's written opinion is still awaited.</li><li>MC option B: a 15% increase plus a one-time ₹1 lakh per flat to the corpus, collected over six months from November. The ₹2.7 Cr (at 92% collection) is placed in deposits, not spent; the model counts only its interest, about ₹11.5 L a year after tax, from May 2027. Drawing on the corpus itself is possible but is the last resort.</li></ol></section>
</div>
</div>
...
const P={asis:{pct:0,inside:false,contrib:false},sc:{pct:10.1,inside:true,contrib:false},mc:{pct:null,inside:false,contrib:false},mcb:{pct:15,inside:false,contrib:false,corpus:true},opt1:{pct:39.37,inside:false,contrib:false},opt2:{pct:0,inside:false,contrib:true}};
...
function run(o){
  ...
  const corpusRaised=o.corpus? 0.92*CORPUS_PER_FLAT*TYPES.reduce((a,t)=>a+t.n,0) : 0;   // ₹ L, collected Nov 2026–Apr 2027, placed in deposits, not spent
  ...
  const collections=monthlyBaseNew*12*0.92/1e5 + (o.contrib&&o.rep2?0.92*contribNet:0);
  const income=collections+25.6+34.3+6+9.6+itcYear-(o.taxQ*4)+(o.corpus?corpusIntNet:0);
  const spend=(o.base==="tr"?33*12+40:26*12+16+40)-o.sav;   // +40 L a year of one-time items, this year's rate
...
let chart;
const KEYS=["asis","sc","mc","mcb","opt1","opt2"];
const NM={asis:"Carry on as we are: +0%",sc:"Sub-Committee: +10.1%",mc:"MC option A:",mcb:"MC option B: +15% and ₹1 L to the corpus",opt1:"GBM Option 1: +39.4%",opt2:"GBM Option 2: annual contribution"};
...
const SHORT={asis:"Carry on",sc:"Sub-Committee +10.1%",mc:"MC option A",mcb:"MC option B: 15% + corpus",opt1:"Option 1 +39.4%",opt2:"Option 2 contribution"};
...
<section><h2>Two things owners should know about the Sobha arrangement</h2>
...
<div class="callout"><strong>Purchases.</strong> Under the working arrangement, Sobha's estate manager buys consumables and spares on the association's account. That is where last year's monthly bulb purchases and above-market prices, noted by the Sub-Committee, came from. The purchasing and accounting policy is being reviewed, and a revised policy will be put to the Special General Body Meeting for approval.</div></section>
<section><h2>A tax point that affects the numbers</h2><p>The association is registered for GST. Security services bought from a proprietorship by a registered body are taxed under "reverse charge": the association must pay the 18% itself, and the agency should not charge it. The agency has been charging it, about ₹1 L a month, and the association has been paying it. The Managing Committee's tax adviser is computing what the association owes directly since November 2025 and what it can recover or set off. The net cost is small; the interest and the paperwork are not. It is mentioned here because it will appear in the accounts and owners should not be surprised by it.</p></section>
<section><h2>The rules being written</h2><ol class="steps"><li>Every contract worth more than ₹25 L a year goes to the General Body, whatever the monthly bill looks like.</li><li>Three quotes above ₹25,000; vendor identity and bank account verified before the first payment; no payments to personal accounts.</li><li>A contract register with renewal dates and a 90-day tender trigger, so nothing rolls over by default again.</li><li>The association issues its own contract form; vendors sign ours.</li></ol></section>"""
    page("contracts.html","Contracts and commitments",con)

    ACT=[("Money and the meeting",[("Special General Body Meeting on Sunday 25 October: notice with the full pack by 30 September","MC","Sep–Oct"),("Reconciled FY 2026–27 budget from the accountant's statements and bank figures, shared with the Sub-Committee for comment first","Treasurer","by 26 Sep"),("Written GST advice on whole-amount vs excess taxation and input credit, before the notice","Tax adviser","by 10 Oct"),("Until the General Body decides, the shortfall is met from the reserves; the amount drawn is reported on the reserves page each month","Treasurer","monthly"),("Close the remaining ₹37 L listing difference on HDFC deposits with a full list from the bank, and obtain ICICI maturity dates","Treasurer","Sep"),("Corpus bridge from ₹11.76 Cr at handover to today's deposits, from the audited balance sheets","Treasurer, auditor","in the pack")]),
    ("Accountability",[("Sub-Committee report sent to the previous Managing Committee for written responses on each point; responses published with the pack","President","by 28 Sep"),("Written questions to the auditor on the corpus withdrawals, the payment without an invoice, payments to personal accounts and the security GST","MC","in progress"),("General Body asked to approve an independent review of the flagged transactions, with a defined scope and budget","MC","SGBM"),("Nobody named or blamed ahead of findings; process and dates communicated instead","MC","standing")]),
    ("Contracts",[("Interim agreement with Sobha for October–March with a rate card and headcount","Secretary","Sep"),("Facility-management tender with service standards from the survey","Secretary, engineering group","before SGBM"),("Security: written position on deployment, penalties applied, retender specification","Secretary","Sep–Oct"),("Lift contract rebid on uptime; monthly lift log published from October","Engineering group","Oct"),("STP: odour audit, retender with standards","Engineering group","Oct")]),
    ("Services residents asked about",[("Complaint standard in MyGate: acknowledged in 4 working hours, updated every 48 hours, closed on your confirmation; monthly numbers published","Secretary","Oct"),("Pest control: vendor's schedule audited, mosquito source reduction, published fogging calendar, drain-line treatment","Secretary","Sep–Oct"),("Play area: hardened sand and exposed pipes inspected and fixed","Engineering group","Sep"),("Corridor and staircase light timers re-commissioned","Secretary","Sep"),("Tennis coaching slots; clubhouse booking through MyGate; gym and AC servicing","Secretary","Oct"),("Arrears: notices to the 30 flats with dues; owners more than 60 days in arrears at year-end cannot vote in or stand for the next election (bye-law 7)","Treasurer","Sep")]),
    ("Rules for the future",[("Financial controls policy: contract thresholds by annual value, three quotes, vendor checks, monthly reporting, accounts published by 31 May","Treasurer","SGBM"),("Corpus policy: interest is income, principal is not; three signatures or five MC signatures; monthly deposit statement","Treasurer","SGBM"),("Monthly transparency pack on MyGate and on these pages by the 10th","Secretary","from Oct"),("Sinking fund from next year for lifts, painting and waterproofing","MC","next AGM")])]
    ...
    act='<section><div class="eyebrow">The MC\'s commitments, in public</div><h1>What the MC is doing</h1><p class="lead">Every item has an owner and a date. Progress is marked here each month. Anything that slips stays on the list with its reason.</p></section>'+arows+'<section><p class="small">"Engineering group" is the volunteer engineering sub-committee of owners. Personal names are not used on these pages.</p></section>'
